...
class BuyOnlineLandingWeb extends GetView<HomeController> {
  const BuyOnlineLandingWeb({super.key});
      ...
      return Scaffold(
        body: CustomScrollView(
          ...
          slivers: [
            ...
            SliverToBoxAdapter(
              child: Container( color:Colors.red,height: (c.overlap == 0 ? 8 : c.overlap + 8),),
            ),

            // BODY: first load shimmer else tab content
            if (c.isFirstLoad.value) ..._buildShimmerBody()
            else  SliverToBoxAdapter(child:  Container(width: 200,height: 3000,color: Colors.red,))
          ],
        ),
      );
    });
